...
<head>
    ...
    <style>
        ...
        .appointment-details button.cancel {
        ...
        .appointment-details button.cancel:hover {
        ...
        .action-buttons button {
            width: 48%;
            margin: 0 1%; /* Add margin for space between buttons */
        }
    </style>
</head>
<body>
    <img src="img/bg1.png" alt="Background" class="background-image">
    ...
    <div class="appointment-details">
        ...
        <%
            ...
            if ("POST".equalsIgnoreCase(request.getMethod())) {
                appointmentId = request.getParameter("selectedAppointmentId");
        ...
        <form method="post" id="cancelForm">
            ...
            <input type="text" id="appointmentId" name="appointmentId" value="<%= appointmentId %>" readonly>
            ...
            <div class="action-buttons">
                <button type="submit" class="cancel" onclick="return validateSelection()">
                    <i class="bi bi-trash-fill"></i> Cancel Appointment
                </button>
                <button type="button" onclick="window.location.href='searchAppointment.jsp'">
        ...
        <table class="appointment-table">
            ...
            <tbody>
                <%
                    try (Connection conn = DriverManager.getConnection(url, user, password)) {
                        String sql = "SELECT appointment_id, patient_id, service_id, appointment_date, appointment_time, status FROM Appointments WHERE status = 'Scheduled' ORDER BY appointment_date, appointment_time";
                        try (Statement stmt = conn.createStatement();
                             ResultSet rs = stmt.executeQuery(sql)) {
                            while (rs.next()) {
                                String apptId = rs.getString("appointment_id");
                                String patId = rs.getString("patient_id");
                                String servId = rs.getString("service_id");
                                String date = rs.getString("appointment_date");
                                String time = rs.getString("appointment_time");
                                String stat = rs.getString("status");

                                out.println("<tr id='row-" + apptId + "'>");
                                out.println("<td><input type='radio' name='appointmentSelector' value='" + apptId + "' onclick=\"selectAppointment('" + apptId + "','" + patId + "','" + servId + "','" + date + "','" + time + "')\"></td>");
                                out.println("<td>" + apptId + "</td>");
                                out.println("<td>" + patId + "</td>");
                                out.println("<td>" + servId + "</td>");
    ...
    <script>
        ...
        function selectAppointment(appointmentId, patientId, serviceId, appointmentDate, appointmentTime) {
